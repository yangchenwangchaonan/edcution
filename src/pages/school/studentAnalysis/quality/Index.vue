               ...
               <div class='right'>
                    ...
                    <div class="edu_knowledge">
                        ...
                        <div class="edu_info">
                            ...
                            <div v-if="question_list1!=null && question_list1!=''">
    ...
    export default {
        ...
        methods: {
            ...
            //切换左边栏
             ...
             // 渲染学科列表
            handleSubjectList(){
                let school = JSON.parse(sessionStorage.getItem('school'))
                let school_id = school[0].school_id
				this.axios.get('/school/'+school_id+'/subject/0/termYear/2018/grade/12/term/1/artSci/2/subject_knowledge_school_study?debug=true',{
                    params:{
                        term_year:2018,
                        grade:12,
                        term:1,
                ...
                .then((res)=> {
                    if (res.data.code==0) {
                        this.nav=res.data.data.subject_infos
                        // 首次 进入 默认this.subject_id
                        this.subject_id = res.data.data.subject_infos[this.navIndex].subject_id
                        this.handleStudyList()
                    }
                })
                .catch((err)=> {
            ...
            handleStudyList () {
                let school = JSON.parse(sessionStorage.getItem('school'))
                let school_id = school[0].school_id
				this.axios.get('/school/'+school_id+'/subject/'+this.subject_id+'/termYear/'+2018+'/grade/'+12+'/term/'+1+'/artSci/'+2+'/paper_knowledge_school_study?debug=true',{
                    params:{
                        term_year: 2018,
                        grade:12,
                        term:1,
                ...
                .then((res)=> {
                    if (res.data.code==0) {
                        // 侧边栏数据
                        this.leftNav=res.data.data.knowledges
                        // 首次  进入 默认this.knowledge_id
                        this.knowledge_id = res.data.data.knowledges[this.leftNavIndex].knowledge_id
                        this.handleKnowledgeInfo()
                    }
                })
                .catch((err)=> {
             ...
             // 渲染知识点详情
            handleKnowledgeInfo () {
                console.log(this.subject_id,this.knowledge_id)
                let school = JSON.parse(sessionStorage.getItem('school'))
                let school_id = school[0].school_id
				this.axios.get('/school/'+school_id+'/subject/'+this.subject_id+'/termYear/2018/grade/12/term/1/artSci/2/knowledge/'+this.knowledge_id+'/detail?debug=true',{
                    params:{
                        term_year: 2018,
                        grade:12,
                        term:1,
                ...
                .then((res)=> {
                    ...
                    if (res.data.code==0) {
                        ...
                        let data = res.data.data 
                         if (data.objective_question_info!="" && data.objective_question_info!=null) {
                        this.exam_count = data.objective_question_info.exam_count
                        this.score_rate = data.objective_question_info.question_type
                        this.question_list = data.objective_question_info.question_list
                         }
                        if (data.subjective_question_info!="" && data.subjective_question_info!=null) {
                        this.score_rate1 = data.subjective_question_info.question_type
